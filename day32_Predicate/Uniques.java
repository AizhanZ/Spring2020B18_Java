...
public class Uniques {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < charList.size(); i++ ){
            int count = Collections.frequency(charList, charList.get(i));//charList.get(i) will get the frequency of first element
            if(count == 1){
        ...
        for(Character each: charList ){ // each: A, A, B, C, D, D
            ...
            int count = Collections.frequency(charList, each);//charList
            if(count == 1){
                result.add( each );
            }
